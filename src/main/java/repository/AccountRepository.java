...
public class AccountRepository implements Repository {
    ...
    public List<Account> showCustomerAccount(int customerId){
        String query = "select a2.id,a2.account_number,a2.balance,c2.number,c2.expirdate,c2.cvv,c2.password,c2.password2,c2.isactive from customer c inner join accoutncustoer a on c.id = a.customer_id\n" +
        ...
        try {
            ...
            preparedStatement.setInt(1,customerId);
             ...
             while (resultSet.next()){
                 CreditCard creditCard = new CreditCard(resultSet.getString(4), resultSet.getString(5),
                         resultSet.getInt(6),
   ...
   public void upDate(Account account){
        String query = "UPDATE account set account_number = ? ,balance = ?,credit_card_id = ?,type_id = ? " +
                "WHERE id = ?;";
       try {
           preparedStatement = connection.prepareStatement(query);
           preparedStatement.setString(1,account.getAccountNumber());
           ...
           preparedStatement.setInt(4,account.getTypeId());
